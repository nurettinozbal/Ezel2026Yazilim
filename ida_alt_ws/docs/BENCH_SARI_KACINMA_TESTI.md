...
- Bridge düzeyindeki bench dönüş tavanı `0.10 rad/s`, izin verilen üst sınır
  `0.15 rad/s` olur. DWA daha sert bir recovery istese bile fiziksel çıkış bu
  zarfı aşamaz; geri hız bench profilinde `0.0 m/s` ile kapalıdır.
...
- Telemetri, obstacle timeout, bridge command timeout ve STOP korumaları aynen
  kalır.
- GUIDED çıkış ayrıca Pixhawk'tan taze `armed`, gerçek `GUIDED`, global konum ve
  yerel konum/EKF sağlığı ister. Köprü başladıktan sonra en az bir DISARM örneği
  görülmeden hareket açılmaz. Aktif döngüde HOLD/EKF/konum/bağlantı kaybı kilidi
  kapatır; yalnız DISARM bu kilidi sıfırlar. Köprü kendi başına GUIDED'a dönmez.
  Not: MAVSDK, ArduPilot Rover'ın custom mode 15 `GUIDED` değerini kendi ortak
  enum'unda `OFFBOARD` diye raporlar; bridge yalnız bu doğrulanmış eşlemeyi kabul
  eder. ArduPilot tarafındaki gerçek mod yine GUIDED'dır.
...
- `guided_actuation.allowed=false` veya `guided_actuation.latched=true`,
- gerçek modun GUIDED dışında olması ya da global/local position health kaybı,
...
## 15 Ağustos kalıcı kalibrasyon kaydı

- Araç merkezinden buruna doğru lidar ofseti `+0.57 m` olarak korunur.
- Kurulu S2 için `angle_offset_deg=180` ve `mirror_scan=true` kullanılır.
- Kontrollü sağ/sol şişe yer değiştirme testinde `mirror=false` lidar bearingini
  kameraya ters yönde taşımış, `mirror=true` aynı fiziksel tarafta birleştirmiştir.
- Bu ayar başka lidar/montaja kopyalanmaz; donanım değişirse merkez-sağ-sol hedef
  testi yeniden yapılır.
