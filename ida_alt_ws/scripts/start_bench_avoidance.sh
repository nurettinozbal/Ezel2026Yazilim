...
CAMERA_DEVICE="$(resolve_field_camera_device "${IDA_BENCH_CAMERA_DEVICE:-auto}")" || exit 4
MODEL_P1P2="${IDA_MODEL_P1P2:-}"
MODEL_P3="${IDA_MODEL_P3:-}"
MAX_SPEED="${IDA_BENCH_MAX_SPEED_MPS:-0.25}"
MAX_YAW_RATE="${IDA_BENCH_MAX_YAW_RATE_RAD_S:-0.10}"
...
if [[ "$ROUTER" == "true" ]]; then
  command -v tty_mavlink_router >/dev/null || {
    echo "RED: tty_mavlink_router bulunamadi; colcon build/source gerekli."
    exit 4
  }
...
echo "max_speed=$MAX_SPEED m/s max_yaw=$MAX_YAW_RATE rad/s stuck_timeout=$STUCK_TIMEOUT s"
...
ros2 launch ida_bringup bench_avoidance.launch.py \
  ...
  bench_max_yaw_rate_rad_s:="$MAX_YAW_RATE" \
  ...
  bench_camera_device:="$CAMERA_DEVICE" \
