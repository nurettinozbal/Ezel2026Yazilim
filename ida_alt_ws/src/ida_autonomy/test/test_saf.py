...
from ida_planning.planner import task_timeout_expired
...
from ida_planning.geo import bearing_deg, local_m_to_latlon
from ida_planning.behavior import NavigationBehaviorArbiter
...
try:
    from ida_autonomy.autonomy_node import (
        AutonomyNode,
        costmap_inflation_speed_for_state,
...
class P2CostmapInflationPolicyTest(unittest.TestCase):
    def test_p2_zero_cap_keeps_fixed_inflation_layer(self):
        self.assertEqual(
            costmap_inflation_speed_for_state(
                "PARKUR_2_AVOIDANCE", 1.6, 0.0
            ),
            0.0,
        )

    def test_p2_cap_bounds_global_inflation_speed(self):
        self.assertAlmostEqual(
            costmap_inflation_speed_for_state(
                "PARKUR_2_AVOIDANCE", 1.6, 0.45
            ),
            0.45,
        )

    def test_p1_behavior_is_unchanged(self):
        self.assertAlmostEqual(
            costmap_inflation_speed_for_state("PARKUR_1_NAV", 1.2, 0.0),
            1.2,
        )
    ...
    def test_non_finite_input_is_rejected(self):
        with self.assertRaises(ValueError):
            costmap_inflation_speed_for_state(
                "PARKUR_2_AVOIDANCE", float("nan"), 0.0
            )


class P3SearchScheduleTest(unittest.TestCase):
    def test_zero_p3_hold_timeout_waits_without_expiring(self):
        self.assertFalse(task_timeout_expired(86400.0, 0.0))
        self.assertTrue(task_timeout_expired(60.1, 60.0))
    ...
    def test_fused_target_becomes_recomputed_virtual_waypoint(self):
        node = AutonomyNode.__new__(AutonomyNode)
        node.telemetry = {"lat": 40.0, "lon": 29.0, "heading_deg": 0.0}
        node._p3_last_target_fix = None
        node._p3_last_tracking_command = None
        node._p3_last_target_distance_m = None
        node.p3_align_distance_m = 5.0
        node.p3_approach_speed_mps = 0.7
        node.p3_lock_min_speed_mps = 0.7
        node.p3_engage_distance_m = 1.2
        node.p3_engage_speed_mps = 0.7
        node.p3_approach_yaw_max_deg_s = 20.0
        node.max_speed = 1.6
        node.yaw_kp = 0.55

        node._remember_p3_virtual_target(
            {"distance": 10.0, "forward_m": 10.0, "lateral_m": 0.0}
        )
        command = node._p3_virtual_target_command()
        self.assertAlmostEqual(command.vx, 0.7)
        self.assertAlmostEqual(command.yaw_rate, 0.0)
        self.assertIn("target_loss_virtual_waypoint", command.action)

        # Kısa model kaybında sanal hedef 5 m içinde de homing yayını
        # korur; doğrulanmış hedefte engage mesafesinin içinde temas itkisi
        # devam eder (eski 1.2 m erken duruş regresyonu).
        lat, lon = local_m_to_latlon(40.0, 29.0, 6.0, 0.0)
        node.telemetry.update({"lat": lat, "lon": lon})
        command = node._p3_virtual_target_command()
        self.assertEqual(command.vx, 0.7)

        lat, lon = local_m_to_latlon(40.0, 29.0, 9.0, 0.0)
        node.telemetry.update({"lat": lat, "lon": lon})
        command = node._p3_virtual_target_command()
        self.assertEqual(command.vx, 0.7)
        self.assertIn("target_loss_virtual_contact_commit", command.action)

        # Hedef tekne düzlemini geçtiğinde kör ileri itki kesilir.
        lat, lon = local_m_to_latlon(40.0, 29.0, 10.2, 0.0)
        node.telemetry.update({"lat": lat, "lon": lon})
        command = node._p3_virtual_target_command()
        self.assertEqual(command.vx, 0.0)

    def test_actual_heading_wrap_counts_complete_positive_rotation(self):
        progress, heading = 0.0, 359.0
        for sample in (1.0, 91.0, 181.0, 271.0, 359.0):
            progress, heading = p3_scan_progress(progress, heading, sample)
        self.assertAlmostEqual(progress, 360.0)

    def test_wave_driven_reverse_rotation_removes_progress(self):
        progress, heading = p3_scan_progress(120.0, 120.0, 100.0)
        self.assertAlmostEqual(progress, 100.0)
        self.assertAlmostEqual(heading, 100.0)

    def test_invalid_heading_sample_is_fail_closed(self):
        with self.assertRaises(ValueError):
            p3_scan_progress(20.0, 10.0, float("nan"))

    def test_search_anchor_is_internal_and_scout_is_two_metres_ahead(self):
        node = AutonomyNode.__new__(AutonomyNode)
        node.telemetry = {"lat": 40.0, "lon": 29.0, "heading_deg": 90.0}
        node.waypoints = [{"lat": 40.1, "lon": 29.1, "parkur": 2}]
        original_waypoints = list(node.waypoints)
        node.p3_search_reposition_m = 2.0
        node._p3_search_anchor = None
        node._p3_search_scout = None
        node._p3_search_phase = "anchor_scan"
        node._p3_scan_last_heading_deg = None
        node._p3_scan_accum_deg = 0.0

        self.assertTrue(node._initialize_p3_search_anchor(90.0))
        scout_north, scout_east = node.goal_body_forward_m(
            {"lat": node._p3_search_scout[0], "lon": node._p3_search_scout[1]}
        )
        self.assertAlmostEqual(scout_north, 2.0, places=2)
        self.assertAlmostEqual(scout_east, 0.0, places=2)
        self.assertEqual(node.waypoints, original_waypoints)

    def test_anchor_scan_advances_only_after_measured_full_turn(self):
        node = AutonomyNode.__new__(AutonomyNode)
        node.telemetry = {"lat": 40.0, "lon": 29.0, "heading_deg": 0.0}
        node.p3_search_reposition_m = 2.0
        node.p3_search_scan_degrees = 360.0
        ...
        node._p3_scan_last_heading_deg = None
        node._p3_scan_accum_deg = 0.0
        self.assertTrue(node._initialize_p3_search_anchor(0.0))

        for heading in (90.0, 180.0, 270.0, 359.0):
            node.telemetry["heading_deg"] = heading
            command, _ = node._p3_anchor_search_command({})
            self.assertIn("anchor_scan", command.action)
        self.assertEqual(node._p3_search_phase, "anchor_scan")

        node.telemetry["heading_deg"] = 0.0
        command, _ = node._p3_anchor_search_command({})
        self.assertEqual(command.action, "p3_anchor_scan_complete")
        self.assertEqual(node._p3_search_phase, "reposition")

    def test_reposition_and_return_use_geometry_without_reverse(self):
        node = AutonomyNode.__new__(AutonomyNode)
        node.telemetry = {"lat": 40.0, "lon": 29.0, "heading_deg": 0.0}
        node.p3_search_reposition_m = 2.0
        node.p3_search_scan_degrees = 360.0
        node.p3_search_yaw_rate_deg_s = 30.0
        node.p3_search_transit_speed_mps = 0.7
        node.p3_search_reposition_arrival_m = 0.5
        node.p3_search_anchor_acceptance_m = 1.5
        ...
        node._p3_scan_accum_deg = 0.0
        self.assertTrue(node._initialize_p3_search_anchor(0.0))
        anchor = node._p3_search_anchor
        scout = node._p3_search_scout
        calls = []

        def guard(command, debug, *, allow_reverse_escape=True):
            calls.append(allow_reverse_escape)
            debug["near_field"] = {"active": False}
            return command, debug

        node._near_field_guarded = guard
        node._p3_search_phase = "reposition"
        command, _ = node._p3_anchor_search_command({})
        self.assertGreater(command.vx, 0.0)
        self.assertEqual(calls, [False])

        node.telemetry.update({"lat": scout[0], "lon": scout[1]})
        command, _ = node._p3_anchor_search_command({})
        self.assertEqual(command.action, "p3_reposition_complete")
        self.assertEqual(node._p3_search_phase, "scout_scan")

        node._set_p3_return_to_anchor()
        node.telemetry.update({"lat": anchor[0], "lon": anchor[1]})
        command, _ = node._p3_anchor_search_command({})
        self.assertEqual(command.action, "p3_anchor_reached")
        self.assertEqual(node._p3_search_phase, "anchor_scan")

    def test_blocked_reposition_scans_in_place_without_reverse_loop(self):
        node = AutonomyNode.__new__(AutonomyNode)
        node.telemetry = {"lat": 40.0, "lon": 29.0, "heading_deg": 0.0}
        node.p3_search_reposition_m = 2.0
        node.p3_search_scan_degrees = 360.0
        ...
        def blocked(_command, debug, *, allow_reverse_escape=True):
            self.assertFalse(allow_reverse_escape)
            debug["near_field"] = {"active": True}
            return Command(0.0, 0.0, 0.2, "hard_obstacle"), debug

        node._near_field_guarded = blocked
        command, _ = node._p3_anchor_search_command({})
        self.assertEqual(command.action, "p3_reposition_blocked_scan")
        self.assertEqual(command.vx, 0.0)
        self.assertEqual(node._p3_search_phase, "scout_scan")
...
class MissionPhaseTransitionTest(unittest.TestCase):
    ...
    def test_zero_duration_disables_p1_and_p2_task_timeouts(self):
        self.assertIsNone(p1_transition_decision(
            yellow_count=0, yellow_threshold=4, route_exit_ready=False,
            all_waypoints_complete=False, elapsed_s=86400.0, max_duration_s=0.0,
        ))
        self.assertIsNone(p2_transition_decision(
            crossed_count=0, min_crossings=4, route_complete=False,
            elapsed_s=86400.0, max_duration_s=0.0,
        ))
    ...
    def test_p2_completion_starts_internal_five_meter_leg_without_new_waypoint(self):
        node = AutonomyNode.__new__(AutonomyNode)
        node.state = "PARKUR_2_AVOIDANCE"
        node.raw_waypoints = [
        ...
        node.waypoints = [dict(item) for item in node.raw_waypoints]
        original = [dict(item) for item in node.raw_waypoints]
        node.telemetry = {
            "lat": 40.7235886,
            "lon": 29.8254987,
            "heading_deg": 145.0,
        }
        node.p2_exit_advance_m = 5.0
        node.p2_exit_advance_speed_mps = 0.70
        node.p2_exit_align_latch_release_deg = 150.0
        node.nav_arbiter = Mock()
        node._stuck_since = 10.0
        node.get_logger = Mock(return_value=Mock())

        self.assertTrue(node.begin_p2_exit_advance(200.0, p2_completed=True))
        self.assertEqual(node.state, "PARKUR_2_AVOIDANCE")
        self.assertEqual(node.raw_waypoints, original)
        self.assertEqual(node.waypoints, original)
        self.assertAlmostEqual(
            node._p2_exit_advance_bearing_deg,
            bearing_deg(
                original[-2]["lat"], original[-2]["lon"],
                original[-1]["lat"], original[-1]["lon"],
            ),
            places=6,
        )
        node.nav_arbiter.reset.assert_called_once_with()

    def test_p2_internal_leg_transitions_only_after_along_track_five_meters(self):
        node = AutonomyNode.__new__(AutonomyNode)
        node.p2_exit_advance_m = 5.0
        node._p2_exit_advance_origin = (40.0, 29.0)
        node._p2_exit_advance_bearing_deg = 90.0
        current_lat, current_lon = local_m_to_latlon(40.0, 29.0, 0.0, 5.1)
        node.telemetry = {
            "lat": current_lat, "lon": current_lon, "heading_deg": 90.0,
        }
        node.transition_to_p3 = Mock(return_value=True)

        command, debug = node.handle_p2_exit_advance(210.0)
        node.transition_to_p3.assert_called_once_with(210.0, p2_completed=True)
        self.assertEqual(command.action, "p2_exit_advance_complete")
        self.assertGreaterEqual(debug["p2_exit_advance"]["progress_m"], 5.0)

    def test_p2_internal_leg_keeps_shared_lidar_collision_envelope(self):
        node = AutonomyNode.__new__(AutonomyNode)
        node.state = "PARKUR_2_AVOIDANCE"
        node.p2_exit_advance_m = 5.0
        node.p2_exit_advance_speed_mps = 0.70
        node.p2_exit_align_latch_release_deg = 150.0
        node._p2_exit_advance_origin = (40.0, 29.0)
        node._p2_exit_advance_bearing_deg = 0.0
        node._p2_exit_align_direction = 0.0
        node.telemetry = {"lat": 40.0, "lon": 29.0, "heading_deg": 0.0}
        node.yaw_kp = 0.55
        node.max_yaw_rate = 50.0
        node._near_field_latch = False
        node.obstacles = [{
            "id": "front", "hard_obstacle": True,
            "forward_m": 1.0, "lateral_m": 0.0, "distance": 1.0,
        }]
        node.nav_arbiter = NavigationBehaviorArbiter(
            high_yaw_deg_s=22.5,
            obstacle_stop_m=2.0,
        ...
        command, debug = node.handle_p2_exit_advance(210.0)
        self.assertEqual(command.vx, 0.0)
        self.assertIn("navigation_safety_stop", command.action)
        self.assertTrue(debug["behavior"]["safety_reason"])

    def test_p2_exit_align_keeps_direction_at_antipodal_heading(self):
        node = AutonomyNode.__new__(AutonomyNode)
        node.state = "PARKUR_2_AVOIDANCE"
        node.p2_exit_advance_m = 5.0
        node.p2_exit_advance_speed_mps = 0.70
        node.p2_exit_align_latch_release_deg = 150.0
        node._p2_exit_advance_origin = (40.0, 29.0)
        node._p2_exit_advance_bearing_deg = 146.0
        node._p2_exit_align_direction = 1.0
        node.telemetry = {"lat": 40.0, "lon": 29.0, "heading_deg": -34.1}
        node.yaw_kp = 0.55
        node.max_yaw_rate = 50.0
        node._near_field_latch = False
        node.obstacles = []
        ...
        command, _debug = node.handle_p2_exit_advance(210.0)
        self.assertGreater(command.yaw_rate, 0.0)
        self.assertEqual(command.action, "p2_exit_align")
...
class P3TrackedEngagementTest(unittest.TestCase):
    def _engage_node(self):
        ...
        node._p3_target_confirmed_latched = True
        node._p3_target_lost_since = None
        node._p3_last_target_fix = None
        node._p3_safety_pause_active = False
        node._p3_safety_pause_started = None
        ...
        node.p3_align_distance_m = 5.0
        node.p3_approach_speed_mps = 0.70
        node.p3_approach_yaw_max_deg_s = 20.0
        node.p3_target_loss_grace_s = 3.5
        node.p3_target_memory_timeout_s = 20.0
        node.yaw_kp = 0.55
        node.obstacles = [{
            "id": "lidar-1", "hard_obstacle": True,
            "forward_m": 0.8, "lateral_m": 0.0, "distance": 0.8,
        }]
        node._near_field_latch = False
        ...
        node.p3_near_field_stop_m = 1.0
        node.p3_near_field_stop_release_m = 1.8
        return node
    ...
    def test_engage_alignment_drift_keeps_smooth_homing(self):
        node = self._engage_node()
        node.buoys = [{
        ...
        node.tick()
        command = node.publish.call_args.args[0]
        self.assertEqual(node.state, "ENGAGE")
        self.assertGreater(command.vx, 0.0)
        self.assertIn("engage_homing target_align_moving", command.action)
        self.assertEqual(node.engage_started, 10.0)

    def test_confirmed_engage_dropout_commits_to_virtual_contact(self):
        node = self._engage_node()
        node.telemetry = {"lat": 40.0, "lon": 29.0, "heading_deg": 0.0}
        node._remember_p3_virtual_target({
            "distance": 0.8, "forward_m": 0.8, "lateral_m": 0.0,
        })
        node.tick()
        command = node.publish.call_args.args[0]
        self.assertEqual(node.state, "ENGAGE")
        ...
        node.now_sec.return_value = 12.0
        node.tick()
        command = node.publish.call_args.args[0]
        self.assertEqual(node.state, "COMPLETE")
        self.assertEqual(command.action, "mission_complete")
    ...
    def test_confirmed_contact_target_is_not_a_near_field_hazard(self):
        node = self._engage_node()
        selected = [{
            "id": "cam-1", "lidar_obstacle_id": "lidar-1",
            "color": "green", "confidence": 0.9, "distance": 0.8,
        }]
        command, debug, paused = node._p3_guard_tracking_command(
            Command(0.7, 0.0, 0.0, "engage"), {}, selected, 10.0
        )
        self.assertEqual(command.vx, 0.7)
        self.assertFalse(paused)
        self.assertEqual(
            debug["p3_safety_pause"]["ignored_target_obstacle_ids"],
            ["cam-1", "lidar-1"],
        )

    def test_unconfirmed_color_does_not_whitelist_obstacle(self):
        node = self._engage_node()
        node._p3_target_confirmed_latched = False
        selected = [{
            "id": "cam-1", "lidar_obstacle_id": "lidar-1",
            "color": "green", "confidence": 0.9, "distance": 0.8,
        }]
        command, _debug, paused = node._p3_guard_tracking_command(
            Command(0.7, 0.0, 0.0, "target_confirming"), {}, selected, 10.0
        )
        self.assertEqual(command.vx, 0.0)
        self.assertTrue(paused)

    def test_other_hard_obstacle_pauses_confirmed_target_approach(self):
        node = self._engage_node()
        node.obstacles.append({
            "id": "blocking-wall", "hard_obstacle": True,
            "forward_m": 0.9, "lateral_m": 0.1, "distance": 0.9,
        })
        selected = [{
            "id": "cam-1", "lidar_obstacle_id": "lidar-1",
            "color": "green", "confidence": 0.9, "distance": 0.8,
        ...
        command, debug, paused = node._p3_guard_tracking_command(
            Command(0.7, 0.0, 0.0, "target_approach"), {}, selected, 10.0
        )
        self.assertEqual(command.vx, 0.0)
        self.assertTrue(paused)
        self.assertTrue(debug["near_field"]["latch"])

    def test_p3_non_target_obstacle_outside_one_meter_does_not_hard_stop(self):
        node = self._engage_node()
        node.obstacles.append({
            "id": "blocking-wall", "hard_obstacle": True,
            "forward_m": 1.65, "lateral_m": 0.1, "distance": 1.65,
        })
        selected = [{
            "id": "cam-1", "lidar_obstacle_id": "lidar-1",
            "color": "green", "confidence": 0.9, "distance": 0.8,
        }]
        command, debug, _paused = node._p3_guard_tracking_command(
            Command(0.7, 0.0, 0.0, "target_approach"), {}, selected, 10.0
        )
        self.assertGreater(command.vx, 0.0)
        self.assertFalse(debug["near_field"]["latch"])
        self.assertEqual(debug["near_field"]["stop_m"], 1.0)
        self.assertEqual(debug["near_field"]["stop_release_m"], 1.8)

    def test_engage_state_survives_safety_pivot_and_camera_dropout(self):
        node = self._engage_node()
        node.buoys = [{
            "id": "cam-1", "lidar_obstacle_id": "lidar-1",
            "color": "green", "confidence": 0.9, "distance": 0.8,
        ...
        self.assertEqual(node.state, "ENGAGE")
        self.assertEqual(command.vx, 0.0)
        self.assertTrue(node._p3_safety_pause_active)

        # Güvenlik pivotunun sonraki karede hedefi FOV dışına atması gerçek
        # target-loss sayılmaz; state ve temas zamanlayıcısı korunur.
        node.now_sec.return_value = 11.0
        node.buoys = []
        node.tick()
        self.assertEqual(node.state, "ENGAGE")
        self.assertTrue(node._p3_safety_pause_active)

    def test_safety_pause_freezes_engage_and_loss_timers(self):
        node = self._engage_node()
        node.engage_started = 5.0
        node._p3_target_lost_since = 7.0
        node._update_p3_safety_pause(True, 10.0)
        node._update_p3_safety_pause(False, 13.0)
        self.assertEqual(node.engage_started, 8.0)
        self.assertEqual(node._p3_target_lost_since, 10.0)
        self.assertFalse(node._p3_safety_pause_active)

    def test_confirmed_target_memory_outlives_short_loss_grace(self):
        node = self._engage_node()
        node._p3_target_lost_since = 10.0
        # 3.5 s komut grace'i bitmiş olsa da sanal fused hedef 20 s boyunca
        # arama/anchor otoritesine bırakılmaz.
        self.assertFalse(node._p3_target_memory_expired(13.6))
        self.assertFalse(node._p3_target_memory_expired(30.0))
        self.assertTrue(node._p3_target_memory_expired(30.1))

    def test_safety_pause_extends_target_memory_budget(self):
        node = self._engage_node()
        node._p3_target_lost_since = 10.0
        node._update_p3_safety_pause(True, 15.0)
        node._update_p3_safety_pause(False, 20.0)
        self.assertEqual(node._p3_target_lost_since, 15.0)
        self.assertFalse(node._p3_target_memory_expired(34.9))
