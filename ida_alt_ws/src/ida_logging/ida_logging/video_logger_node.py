...
class VideoLoggerNode(Node):
    ...
    def tick(self) -> None:
        ...
        if time.monotonic() - self._last_frame_monotonic >= self.frame_stall_timeout_s:
            ...
            # Fiziksel kamera koparsa logger çalışmaya devam edebilir. Bu anda
            # yalnız segmentleri değil, hakeme verilecek tek MP4'ü de atomik
            # üret; kamera geri gelirse sonraki segmentler shutdown/stall'da
            # yeniden birleşir. Normal 10 s rotasyonda büyüyen MP4 tekrar tekrar
            # yazılmaz ve Jetson depolaması gereksiz yıpratılmaz.
            self._finalize_delivery("camera_stall")

    def _finalize_delivery(self, reason: str) -> None:
        try:
            output = finalize_delivery_video(self._run_dir)
            if output is None:
                self.get_logger().warn(
                    f"Tek processed_video.mp4 uretilemedi ({reason}); "
                    "finalize edilmis segmentler korunuyor"
                )
            else:
                self.get_logger().info(
                    f"Teslim videosu hazir ({reason}): {output}"
                )
        except Exception as exc:
            self.get_logger().error(
                f"Teslim videosu birlestirilemedi ({reason}); "
                f"segmentler korunuyor: {exc}"
            )
    ...
    def destroy_node(self) -> None:
        ...
        self._finalize_delivery("shutdown")
        super().destroy_node()
