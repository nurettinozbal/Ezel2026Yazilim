...
class TestVideoSegments(unittest.TestCase):
    def test_closed_segment_is_atomically_named_and_journaled(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp) / "run_video"
            run_dir.mkdir()
            journal = SegmentJournal(run_dir)
            index, partial, final = journal.allocate()
            partial.write_bytes(b"finalized-mp4-data")
            committed = journal.commit(
                partial,
                final,
                {"segment_index": index, "frame_count": 150, "close_reason": "duration"},
            )
            self.assertFalse(partial.exists())
            self.assertEqual(committed, final)
            self.assertEqual(committed.read_bytes(), b"finalized-mp4-data")
            record = json.loads(journal.manifest_path.read_text(encoding="utf-8"))
            self.assertEqual(record["frame_count"], 150)
            self.assertEqual(record["close_reason"], "duration")
            self.assertEqual(record["file"], "processed_video_segments/processed_video_000001.mp4")

    def test_restart_allocates_after_existing_segment_without_overwrite(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp) / "run_video"
            run_dir.mkdir()
            first = SegmentJournal(run_dir)
            _, partial, final = first.allocate()
            partial.write_bytes(b"one")
            first.commit(partial, final, {"segment_index": 1})
            second = SegmentJournal(run_dir)
            index, _, next_final = second.allocate()
            self.assertEqual(index, 2)
            self.assertTrue(next_final.name.endswith("000002.mp4"))

    def test_map_segment_uses_independent_names_and_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp) / "run_map"
            run_dir.mkdir()
            journal = SegmentJournal(run_dir, stem="map")
            index, partial, final = journal.allocate()
            partial.write_bytes(b"map-segment")
            journal.commit(partial, final, {"segment_index": index, "frame_count": 5})
            self.assertEqual(final.parent.name, "map_segments")
            self.assertEqual(final.name, "map_000001.mp4")
            record = json.loads(journal.manifest_path.read_text(encoding="utf-8"))
            self.assertEqual(record["file"], "map_segments/map_000001.mp4")

    def test_delivery_video_replaces_output_only_after_success(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp) / "run_video"
            run_dir.mkdir()
            journal = SegmentJournal(run_dir)
            _, partial, final = journal.allocate()
            partial.write_bytes(b"segment")
            journal.commit(partial, final, {"segment_index": 1})
            output = run_dir / "processed_video.mp4"
            ...
            def successful_runner(argv, **_kwargs):
                Path(argv[-1]).write_bytes(b"combined-output")
                return Result()

            combined = finalize_delivery_video(
                run_dir, runner=successful_runner, ffmpeg_path="ffmpeg-test"
            )
            self.assertEqual(combined, output)
            self.assertEqual(output.read_bytes(), b"combined-output")

    def test_failed_delivery_keeps_previous_valid_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp) / "run_video"
            run_dir.mkdir()
            journal = SegmentJournal(run_dir)
            ...
            failed = finalize_delivery_video(
                run_dir,
                runner=lambda *_args, **_kwargs: Result(),
                ffmpeg_path="ffmpeg-test",
            )
            self.assertIsNone(failed)
            self.assertEqual(output.read_bytes(), b"old-valid-output")

    def test_map_delivery_uses_atomic_map_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = Path(tmp) / "run_map"
            run_dir.mkdir()
            journal = SegmentJournal(run_dir, stem="map")
            _, partial, final = journal.allocate()
            partial.write_bytes(b"map-segment")
            journal.commit(partial, final, {"segment_index": 1, "frame_count": 5})

            class Result:
                returncode = 0

            def successful_runner(argv, **_kwargs):
                Path(argv[-1]).write_bytes(b"combined-map")
                return Result()
            ...
            self.assertEqual(output, run_dir / "map.mp4")
            self.assertEqual(output.read_bytes(), b"combined-map")
...
class TestJudgeContract(unittest.TestCase):
    ...
    def test_status_accepts_journalled_segment_before_delivery_concat(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "telemetry.csv").write_text("stamp,lat\n1.0,40.0\n", encoding="utf-8")
            (root / "map.mp4").write_bytes(b"map")
            (root / "frames.csv").write_text(
                "stamp,frame_index\n1.0,1\n", encoding="utf-8"
            )
            journal = SegmentJournal(root)
            _, partial, final = journal.allocate()
            partial.write_bytes(b"closed-segment")
            journal.commit(partial, final, {"segment_index": 1, "frame_count": 1})
            status = evaluate_referee_files(root)
            self.assertTrue(status["active"])
            video = status["file_status"]["processed_video.mp4"]
            self.assertFalse(video["delivery_ready"])
            self.assertTrue(video["recoverable_segments_ready"])

    def test_status_accepts_journalled_map_segment_before_delivery_concat(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "telemetry.csv").write_text(
                "stamp,lat\n1.0,40.0\n", encoding="utf-8"
            )
            (root / "processed_video.mp4").write_bytes(b"camera")
            (root / "frames.csv").write_text(
                "stamp,frame_index\n1.0,1\n", encoding="utf-8"
            )
            journal = SegmentJournal(root, stem="map")
            _, partial, final = journal.allocate()
            partial.write_bytes(b"closed-map-segment")
            journal.commit(partial, final, {"segment_index": 1, "frame_count": 5})
            status = evaluate_referee_files(root)
            self.assertTrue(status["active"])
            local_map = status["file_status"]["map.mp4"]
            self.assertFalse(local_map["delivery_ready"])
            self.assertTrue(local_map["recoverable_segments_ready"])
