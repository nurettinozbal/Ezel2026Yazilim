...
def _validate(context):
    ...
    if values["canonical_takeover_enabled"] == "true":
        pixhawk = LaunchConfiguration("pixhawk_serial_port").perform(context)
        lidar = LaunchConfiguration("lidar_serial_port").perform(context)
        if values["mavlink_router_enabled"] == "true":
            router = (
                Path(get_package_prefix("ida_control"))
                / "lib"
                / "ida_control"
                / "tty_mavlink_router"
            )
            if not router.is_file():
                raise RuntimeError(
                    f"tty_mavlink_router bulunamadı: {router}; build/source gerekli"
                )
            if not Path(pixhawk).exists():
                raise RuntimeError(f"Pixhawk TTY bulunamadı: {pixhawk}")
        if not Path(lidar).exists():
            raise RuntimeError(f"S2 lidar seri aygıtı bulunamadı: {lidar}")
        baud = LaunchConfiguration("lidar_serial_baudrate").perform(context)
        mode = LaunchConfiguration("lidar_scan_mode").perform(context)
        if baud != "1000000":
            raise ValueError("S2 lidar_serial_baudrate tam olarak 1000000 olmalı")
        if mode not in {"DenseBoost", "Standard", "Sensitivity"}:
            raise ValueError("lidar_scan_mode DenseBoost/Standard/Sensitivity olmalı")
    return []
