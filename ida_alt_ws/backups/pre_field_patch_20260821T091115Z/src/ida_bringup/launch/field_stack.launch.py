...
def _load_field_profile():
    configured = os.environ.get("IDA_FIELD_PROFILE_PATH", "").strip()
    path = (
        Path(configured).expanduser()
        if configured
        else Path(get_package_share_directory("ida_bringup"))
        / "config"
        / "field_profile.yaml"
    )
    if configured and not path.is_absolute():
        raise ValueError("IDA_FIELD_PROFILE_PATH must be absolute")
    path = path.resolve()
    if not path.is_file():
        raise ValueError(f"field profile does not exist: {path}")
