...
class DwaPlanner:
    ...
    def score_candidate(
        ...
        if corr and corr.get("active"):
            ...
            # Adayın sim_time_s sonundaki yanal pozisyonu (vx,yaw integrali).
            y_end = cand.vx * self.sim_time_s * math.sin(cand.yaw_rate * self.sim_time_s / 2.0)
            # Koridor merkezinden yanal sapma (hedef: 0'a yakın = koridorda).
            lateral_err = abs(y_end - center_lateral)
            # Koridor genişliği ~3.5m -> yarı genişlik 1.75m; sapma azaldıkça skor artar.
    ...
    def _front_wall_blocked(costmap: CostMap) -> bool:
        """Yakın önde, araç genişliğini kaplayan sert engel var mı?"""
        # 0.76 m gövde + dinamik costmap şişirmesi göz önüne alındığında üç
        # yanal örneğin aynı ileri bantta çarpışması tek dubadan çok duvar/
        # geniş engel işaretidir. İki ileri kesitten herhangi birinin tamamen
        # kapanması ileri recovery'yi yasaklar.
        for forward in (0.75, 1.25, 1.75):
            if all(costmap.collides(forward, lateral) for lateral in (-0.75, 0.0, 0.75)):
                return True
        return False
