...
class AutonomyNode(Node):
        ...
        if self.state == "PARKUR_1_NAV":
            ...
            if not self.target_color:
                ...
                if not target_visible and (
                    ...
                    if command.vx > 0.0:
                        # Geçici hedef yalnız yön hafızasıdır; lidarın sert
                        # engel kararı her zaman daha yüksek otoritededir.
                    ...
                    if target_visible and str(command.action).startswith("target_approach"):
    ...
    def _p3_virtual_target_command(self) -> Command:
        ...
        stale motor command. Forward motion is allowed only while the estimated
        target remains outside the normal 5 m alignment zone.
        """
        ...
        forward_allowed = forward_m > 0.0 and distance_m > self.p3_align_distance_m
        vx = (
            clamp(self.p3_approach_speed_mps, 0.0, self.max_speed)
            if forward_allowed
            else 0.0
        )
        return Command(
            ...
            0.0,
            math.radians(yaw_rate_deg_s),
            f"target_loss_virtual_waypoint distance={distance_m:.1f}",
        )
