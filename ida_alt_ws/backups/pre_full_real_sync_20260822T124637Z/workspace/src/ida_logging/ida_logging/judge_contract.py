...
def _has_finalized_video_segment(root: Path) -> bool:
    """A journalled non-empty segment is recoverable camera evidence."""
    manifest = root / "processed_video_segments.jsonl"
    try:
        with manifest.open("r", encoding="utf-8", errors="replace") as stream:
            for _ in range(4096):
                ...
                if candidate.parent != root / "processed_video_segments":
                    continue
                if not candidate.name.startswith("processed_video_") or candidate.suffix != ".mp4":
                    continue
...
def evaluate_referee_files(run_dir: str | Path) -> Dict[str, Any]:
    ...
    map_ready = _file_size(local_map) > 0
    frames_ready = _has_data_after_header(frames)
    delivery_video_ready = _file_size(video) > 0
    segment_video_ready = _has_finalized_video_segment(root)
