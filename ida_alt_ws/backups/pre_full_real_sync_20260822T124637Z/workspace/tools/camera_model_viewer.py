...
GENERAL_NAMES = ["black", "green", "orange", "red", "yellow"]
ROLE_LABELS = {
...
def detection_label_color(name: str, box_color: tuple[int, int, int]) -> tuple[int, int, int]:
    """Keep black-class confidence text readable without changing its box colour."""
    return (255, 255, 255) if name.lower() == "black" else box_color
...
def draw_detections(frame: Any, detections: list[dict[str, Any]], cv2: Any, fov_deg: float) -> Any:
    ...
    for detection in detections:
        ...
        _put_text(
            cv2, result, label, x1, max(22, y1 - 7),
            detection_label_color(name, color),
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional five-class general model. When set, the same model is used "
            "for both roles and detections are filtered by the active role."
        ),
    )
...
def main() -> int:
    ...
    general_path = Path(args.model_general) if args.model_general else None
    model_paths = {
        "p12": general_path or Path(args.model_p12),
        "p3": general_path or Path(args.model_p3),
    }
    ...
    def activate(role: str) -> LoadedRole:
        ...
        result = load_model(
            str(path),
            GENERAL_NAMES if general_path is not None else ROLE_NAMES[role],
            args.conf,
            args.iou,
            args.imgsz,
            args.device,
            ...
            if not paused:
                ...
                try:
                    ...
                    if general_path is not None:
                        allowed = set(ROLE_NAMES[role])
                        detections = [
                            detection
                            for detection in detections
                            if detection.get("class_name") in allowed
                        ]
                    inference_error = ""
