...
def shell_lines(resolved: Mapping[str, Any]) -> list[str]:
    ...
    return [
        ...
        ",".join(p1["class_names"]), (json.dumps(p1["class_name_map"], separators=(",", ":")) if p1["class_name_map"] else ""),
        ",".join(resolved["p1p2_allowed_colors"]), str(p3["path"]), str(p3["sha256"]),
        ",".join(p3["class_names"]), (json.dumps(p3["class_name_map"], separators=(",", ":")) if p3["class_name_map"] else ""),
