...
PIXHAWK_PORT="$(resolve_field_serial_device pixhawk "${IDA_PIXHAWK_PORT:-auto}")" || {
  echo "RED: Pixhawk seri aygiti benzersiz bulunamadi." >&2
  exit 4
}
LIDAR_PORT="/dev/ida-lidar-not-present"
if [[ "$LIDAR_ENABLED" == "true" ]]; then
  if ! LIDAR_PORT="$(resolve_field_serial_device lidar "${IDA_LIDAR_PORT:-auto}")" \
      || [[ ! -e "$LIDAR_PORT" ]]; then
...
if [[ "$CAMERA_DRIVER" == "true" ]]; then
  if ! CAMERA_DEVICE="$(resolve_field_camera_device "${IDA_FIELD_CAMERA_DEVICE:-auto}")" \
