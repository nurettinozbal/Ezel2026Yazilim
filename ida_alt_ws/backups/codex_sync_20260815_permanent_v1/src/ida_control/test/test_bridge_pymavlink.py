...
class _Node:
    ...
    def get_parameter(self, name):
        ...
        defaults = {
            ...
            "mission_p1_count_param": "SCR_USER5",
            "mission_p2_count_param": "SCR_USER6",
...
class TestMissionPayload(unittest.TestCase):
    ...
    def test_poll_mission_publishes_json_not_python_repr(self) -> None:
        ...
        async def _get_param(name):
            return 1.0 if name == "SCR_USER5" else 0.0
...
class TestMavsdkTaskTelemetryFirst(unittest.TestCase):
    ...
    def test_connect_failure_still_starts_telemetry_tasks(self) -> None:
        ...
        # position, attitude, velocity, armed, target_color (+mission raw default).
...
class TestRunMavsdkKeepsTasksAlive(unittest.TestCase):
    ...
    def test_run_mavsdk_keeps_background_tasks_alive(self) -> None:
        ...
        node = mavsdk_bridge_node.MavsdkBridgeNode()
        node.mission_raw_enabled = True  # _poll_mission da başlatılsın (6 task)
        ...
        for name in ("position", "attitude", "velocity", "armed"):
            setattr(
                node,
                "_telemetry_" + name,
        ...
        self.assertEqual(len(ran), 6, "6 telemetri/poll task'ı çalışmaya başlamalı")
...
class TestCommandLoopPymavlink(unittest.TestCase):
    ...
    def test_set_guided_mode_does_not_write_vehicle_parameters(self) -> None:
        """GUIDED mode selection is independent from explicit servo setup."""
        self.node._set_guided_mode(self.conn)
        # set_mode_send çağrıldı — custom_mode 15 (GUIDED)
        sent = [c for c in self.conn.calls if c.get("cmd") == "set_mode_send"]
        self.assertTrue(sent, "set_mode_send çağrılmalı (GUIDED)")
        self.assertEqual(sent[0].get("custom_mode"), 15)
        self.assertEqual(sent[0].get("source_system"), 255)
        self.assertEqual(self.conn.mav.srcSystem, 1)
        self.conn.mav.param_set_send.assert_not_called()
    ...
    def test_send_error_triggers_reconnect(self) -> None:
        ...
        # _command_loop_pymavlink kendisi mod geçişi YAPMAZ (o mavsdk_task'un işi);
        # reconnect yolunda _set_guided_mode çağrıldı -> AUTO_MAV:15 (GUIDED).
        self.assertEqual(self.conn.mode_calls, ["AUTO_MAV:15"])  # conn.mav.set_mode_send (GUIDED=15)
