...
PERCEPTION_UNIT = ROOT / "systemd" / "ida-yki-perception.service.in"
...
class FieldStackContractTests(unittest.TestCase):
    def test_linux_install_and_deploy_restore_executable_bits(self):
        ...
        self.assertIn("field-test.local.env", install)
        self.assertIn("ida-yki-perception.service", install)
    ...
    def test_field_yaml_uses_one_enforced_drive_envelope(self):
        ...
        self.assertEqual(result["max_speed_mps"], FIELD_ABSOLUTE_MAX_SPEED_MPS)
        self.assertEqual(result["min_navigation_command_mps"], 0.70)
        for name in (
    ...
    def test_field_drive_envelope_rejects_ineffective_or_over_limit_values(self):
        ...
        for key, value in (
            ("wrong_avoid_speed_mps", 0.69),
            ("p3_engage_speed_mps", 0.69),
            ("dwa_recovery_vx", 1.61),
        ):
        ...
        invalid = copy.deepcopy(original)
        invalid["ida_field_envelope"]["hard_max_speed_mps"] = 1.61
        with self.assertRaises(ValueError):
            validate_field_drive_envelope(invalid)
    ...
    def test_launch_uses_one_general_inference_and_role_filters(self):
        ...
        self.assertIn("IDA_SINGLE_GENERAL_MODEL_ENABLED=true", env)
        model_lines = [
            line.split("=", 1)[1]
            for line in env.splitlines()
            if line.startswith(("IDA_MODEL_P1P2=", "IDA_MODEL_P3="))
        ]
        self.assertEqual(len(model_lines), 2)
        self.assertEqual(model_lines[0], model_lines[1])
        self.assertIn("IDA_MODEL_P1P2_ALLOWED_COLORS=orange,yellow", env)
        self.assertIn("IDA_MODEL_P3_ALLOWED_COLORS=red,green,black", env)
    ...
    def test_yki_perception_stream_is_independent_read_only_and_restartable(self):
        unit = PERCEPTION_UNIT.read_text(encoding="utf-8")
        configure = (ROOT / "scripts" / "configure_yki_perception_stream.sh").read_text(encoding="utf-8")
        self.assertIn("vehicle_test_producer", unit)
        self.assertIn("Restart=always", unit)
        self.assertIn("EnvironmentFile=/etc/ida/field-test.local.env", unit)
        self.assertIn("enable --now ida-yki-perception.service", configure)
        self.assertIn("IDA_YKI_DEBUG_ENABLED=false", configure)
        self.assertIn("auto://yki", configure)
        for forbidden in ("mavsdk", "pymavlink", "cmd_vel", "ARM", "mission/start"):
    ...
    def test_lidar_nodes_respawn_and_field_profile_is_single_tuning_source(self):
        ...
        self.assertIn("dwa_align_before_drive_deg: 42.0", profile_text)
        self.assertIn("p3_search_yaw_rate_deg_s: 30.0", profile_text)

    def test_p2_uses_lidar_primary_camera_secondary_policy(self):
        field_text = FIELD_PROFILE.read_text(encoding="utf-8")
        sim_text = (
            ROOT / "src" / "ida_bringup" / "config" / "autonomy.yaml"
        ).read_text(encoding="utf-8")
        for profile_text in (field_text, sim_text):
            self.assertIn("corridor_bias_parkurs: [1]", profile_text)
            self.assertRegex(profile_text, r"dwa_w_avoid:\s*0\.25\b")
        costmap_text = (
        ...
        self.assertRegex(dwa_text, r'"avoid":\s*0\.25\b')
    ...
    def test_camera_defaults_match_bench_viewer_geometry_and_controls(self):
        ...
        env = ENV.read_text(encoding="utf-8")
        start = START.read_text(encoding="utf-8")
        profile_text = FIELD_PROFILE.read_text(encoding="utf-8")
        for setting in (
            "auto_exposure: 0",
            ...
            "contrast: 10",
            "fov_deg: 82.0",
            "focal_length_px: 552.2",
        ):
            self.assertIn(setting, profile_text)
