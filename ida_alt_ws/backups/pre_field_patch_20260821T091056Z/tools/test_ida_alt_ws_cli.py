...
class IdaAltCliTests(unittest.TestCase):
    def test_help_is_short_and_lists_expected_tools(self) -> None:
        ...
        for command in ("start", "stop", "status", "logs", "cam", "model", "map", "fusion", "gps", "pulse", "yki", "unit"):
    ...
    def test_installed_cli_can_resolve_root_protected_workspace_env(self) -> None:
        ...
        self.assertIn("as_root test -s /etc/ida/field-test.local.env", text)
    ...
    def test_live_profile_opens_gates_but_forbids_vehicle_setup(self) -> None:
        ...
        self.assertIn("max_speed_mps: 1.6", profile)

    def test_start_requires_lidar_and_starts_passive_yki_without_fixed_ip(self) -> None:
        ...
        self.assertIn('systemctl start "$PERCEPTION_SERVICE"', cli)
        self.assertIn('systemctl stop "$PERCEPTION_SERVICE"', cli)
        self.assertIn("IDA_YKI_DEBUG_WEBSOCKET_URL=auto://yki", installer)
        self.assertIn("jetson-perception-token.txt", installer)
